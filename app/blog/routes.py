...
from app.models import Post, Category, Tag, PostStatus, Setting, SettingType
...
def get_posts_by_category(category):
    ...
    posts = (
        Post.query.filter(
            Post.status == PostStatus.PUBLISH.name,
            Post.categories.any(Category.slug == category),
        )
        .order_by(desc("id"))
        .paginate(page=page, per_page=int(ROWS_PER_PAGE))
    )
...
def get_posts_by_tag(tag):
    ...
    posts = (
        Post.query.filter(
            Post.status == PostStatus.PUBLISH.name, Post.tags.any(Tag.slug == tag)
        )
        .order_by(desc("id"))
        .paginate(page=page, per_page=int(ROWS_PER_PAGE))
    )
...
def get_post_by_slug(slug):
    ...
    for h2 in h2tags:
        post.content = post.content.replace(
            f"<h2>{h2}</h2>",
            f'<h2 id="{ h2.replace(" ", "-") }"><a href="#{ h2.replace(" ", "-") }" class="headerlink" title="{h2}"></a>{h2}</h2>',
        )
...
def sitemap_get_post_by_slug():
    ...
    for post in posts:
        yield "blog.get_post_by_slug", {"slug": post.slug}, post.updated_at.strftime(
            "%Y-%m-%dT%H:%M:%SZ"
        ), "weekly", 0.9
